        ...
        for row in raw_schema:
            ...
            try:
                null_count = conn.execute(
                    f"SELECT COUNT(*) FROM data WHERE {quoted_col} IS NULL"
                ).fetchone()[0]

                col_info["null_pct"] = round((null_count / row_count * 100), 2)
            except Exception:
                col_info["null_pct"] = 0.0
            ...
            if is_numeric:
                try:
                    stats = conn.execute(
                        f"""
                        SELECT
                            AVG(CAST({quoted_col} AS DOUBLE)),
                            MIN(CAST({quoted_col} AS DOUBLE)),
                            MAX(CAST({quoted_col} AS DOUBLE))
                        FROM data
            ...
            # ── Boolean stats ──────────────────────────────────────────────
                ...
                try:
                    stats = conn.execute(
                        f"""
                        SELECT
                            SUM(CASE WHEN {quoted_col} = TRUE THEN 1 ELSE 0 END),
                            SUM(CASE WHEN {quoted_col} = FALSE THEN 1 ELSE 0 END)
                        FROM data
                        """
                    ).fetchone()
            ...
            try:
                unique_count = conn.execute(
                    f"SELECT COUNT(DISTINCT {quoted_col}) FROM data"
                ).fetchone()[0]
