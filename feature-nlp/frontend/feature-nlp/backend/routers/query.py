...
Orchestrates: local NLP2SQL generation → DuckDB execution → preprocessing (Smart) →
...
async def run_query(req: QueryRequest) -> QueryResponse:
    ...
    # ── 3. Generate SQL ───────────────────────────────────────────────────
    try:
        sql = generate_sql_query(req.question, schema_columns=schema)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=f"NLP2SQL generation failed: {exc}")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Local NLP2SQL generation failed: {exc}")

    # Basic safety guard — ensure it's a SELECT
    if not _is_select(sql):
        raise HTTPException(
            status_code=422,
            detail=f"NLP2SQL generated a non-SELECT statement. Query rejected.\nSQL: {sql}",
        )
