    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Card className="xl:col-span-2">
          ...
          <p className="mt-1 text-sm text-slate-400">Enter borrower profile values to estimate default probability.</p>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">Credit Inquiries (6mo)</label>
              <Input
                value={form.inqLast6Mths}
                onChange={(event) => setForm((prev) => ({ ...prev, inqLast6Mths: event.target.value }))}
                type="number"
              />
            </div>
            <div>
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400">Public Records</label>
              <Input
                value={form.pubRec}
                onChange={(event) => setForm((prev) => ({ ...prev, pubRec: event.target.value }))}
                type="number"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Input value={form.dti} onChange={(event) => setForm((prev) => ({ ...prev, dti: event.target.value }))} type="number" />
            </div>
            <div>
        ...
        <Card>
          <h3 className="text-lg font-semibold text-white">Prediction Output</h3>
          {!result && !submitting ? (
            <p className="mt-3 text-sm text-slate-400">Run a prediction to see probability, risk label, and confidence meter.</p>
          ) : null}
          {submitting ? <Skeleton className="mt-4 h-40" /> : null}
          {result ? (
            <div className="mt-4 space-y-4">
              <p className="text-5xl font-bold text-cyan-200">{(result.probability * 100).toFixed(1)}%</p>
              <Badge variant={labelVariant(result.label)}>{result.label} Risk</Badge>
              <div>
                <p className="mb-2 text-xs text-slate-400">Risk Gauge</p>
                <Progress value={result.probability * 100} />
              </div>
              <p className="text-xs text-slate-400">Decision threshold: 35% (medium), 65% (high)</p>
            </div>
          ) : null}
        </Card>
      </div>
      ...
      <Card>
        <h3 className="text-lg font-semibold text-white">Feature Importance Explanation</h3>
        <p className="mt-1 text-sm text-slate-400">
          Relative contribution of key borrower factors to the current risk score.
        </p>
        <div className="mt-4">
          {submitting ? <Skeleton className="h-[300px]" /> : null}
          {result ? <FeatureImportanceChart data={result.explainability} /> : null}
          {!result && !submitting ? (
            <p className="rounded-xl border border-dashed border-white/20 bg-slate-900/30 p-6 text-sm text-slate-400">
              No explainability data yet. Run a prediction to populate this panel.
            </p>
          ) : null}
        </div>
      </Card>
